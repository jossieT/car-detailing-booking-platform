...
export default function DashboardPage() {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      if (!statsRes.ok || !bookingsRes.ok) throw new Error('Failed to fetch dashboard data');
            ...
            <tbody className="divide-y divide-white/5">
              {recentBookings.map((booking) => (
                <tr key={booking.id} className="hover:bg-white/5 transition">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-white">${booking.amount.toFixed(2)}</td>
